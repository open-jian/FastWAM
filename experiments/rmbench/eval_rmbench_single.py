#!/usr/bin/env python3
"""Launch one official RM-Bench task with the released RoboTwin FastWAM policy."""
...
def _resolve_official_result_tag(cfg: DictConfig, ckpt_tag: str) -> str:
    ...
    tag = (
        str(configured_tag).strip()
        if configured_tag is not None
        else f"{ckpt_tag}_seed{cfg.seed}"
    )
...
def main(cfg: DictConfig) -> None:
    ...
    policy_source = PROJECT_ROOT / "experiments" / "rmbench" / POLICY_NAME
    if not policy_source.is_dir():
        raise FileNotFoundError(f"Policy source directory not found: {policy_source}")
    _ensure_policy_symlink(
        robotwin_root=rmbench_root,
        policy_source_dir=policy_source.resolve(),
    )
    ...
    # The model architecture/data contract must remain the released RoboTwin
    # one. RM-Bench only supplies the online environment and observations.
    ...
    for key, value in (
        ...
        (
            ...
            bool(cfg.EVALUATION.skip_get_obs_within_replan),
        ),
    ):
    ...
    env["DIFFSYNTH_MODEL_BASE_PATH"] = str(
    ...
    try:
        ...
        if len(new_results) > 1:
            raise RuntimeError(
                f"RM-Bench produced multiple result directories: {new_results}"
            )
        if new_results:
            destination = output_dir / "official_result"
            if destination.exists():
                raise FileExistsError(
                    f"Refusing to overwrite result destination: {destination}"
                )
            shutil.move(str(new_results[0]), str(destination))

    if return_code != 0:
        raise RuntimeError(
            f"RM-Bench evaluation failed with return code {return_code}. Log: {log_path}"
        )
